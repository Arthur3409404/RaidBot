...
```bash
uv run python training/tagteam_arena_model/train_tagteam_model.py --data_path "data/database_champions/datasets/enemy_dataset_tagteam_arena.npz" --inspect_only
```

Dry run:

```bash
uv run python training/tagteam_arena_model/train_tagteam_model.py --data_path "data/database_champions/datasets/enemy_dataset_tagteam_arena.npz" --dry_run
```

Train:

```bash
uv run python training/tagteam_arena_model/train_tagteam_model.py \
  --data_path "data/database_champions/datasets/enemy_dataset_tagteam_arena.npz" \
  --output_dir "training/tagteam_arena_model/outputs" \
  --batch_size 32 \
  --num_workers 8 \
  --pin_memory \
  --epochs 200 \
...
Artifacts are saved under `training/tagteam_arena_model/outputs/` by default:
