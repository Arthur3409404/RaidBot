...
class RSL_Bot_Dungeons:
    ...
    def __init__(
        ...
        defaults_available=["fire_knight", "dragon", "spider", "ice_golem", "sand_devil", "shogun", "event_dungeon"],
        difficulty='hard',
    ...
    def _read_battle_result_once(self):
        ...
        try:
            pov_text_objects = image_tools.get_text_in_relative_area(
                self.reader,
                self.window,
                search_area=self.search_areas["pov"],
            )
            for text_object in pov_text_objects:
                text = (text_object.text or "").strip()
            ...
            if current_setup:
                ...
                if not completed:
                    window_tools.click_at(
                        current_setup.mean_pos_x - 268.0,
                        current_setup.mean_pos_y + 70,
                        delay=2,
                        window=self.window,
                    )
                if self.verbose:
