...
DEFAULT_YOLO_MODEL_CANDIDATES = [
    Path("runs") / "detect" / "data" / "detector_ai" / "runs" / "grimforest_detector-3" / "weights" / "best.pt",
    Path("runs") / "detect" / "data" / "detector_ai" / "runs" / "grimforest_detector-3" / "weights" / "last.pt",
    Path("runs") / "detect" / "data" / "detector_ai" / "runs" / "grimforest_detector" / "weights" / "best.pt",
    Path("runs") / "detect" / "data" / "detector_ai" / "runs" / "grimforest_detector" / "weights" / "last.pt",
]
...
def _load_yolo_detector(model_path: str | Path | None):
    try:
        from data.detector_ai.yolo_detector import YoloDetector
    except Exception:
        return None
...
class RSL_Bot_GrimForest:
    def __init__(self, title_substring="Raid: Shadow Legends", reader=None, window=None, verbose=True, setup=None):
        ...
        self.setup = {
            ...
            "max_random_repositions_when_no_candidates": 9,
            "target_hex": "CEC329",
            "dark_tolerance": 40,
            "reference_dir": str(Path("pic") / "grimforest"),
    ...
    def is_in_grim_forest_mode(self) -> bool:
    ...
    def _move_random_direction_once(self):
        ...
        self.log.info("[Grim Forest] No candidates. Moving randomly: %s.", direction)
    ...
    def detect_candidates_with_random_reposition(self, difficulty: str | None = None) -> list[dict]:
        max_moves = max(0, int(self.setup.get("max_random_repositions_when_no_candidates", 9)))
        for index in range(max_moves + 1):
            ...
            if candidates:
                return candidates
            if index < max_moves and self.main_loop_running:
    ...
    def select_grim_forest_candidate(self, candidate: dict) -> bool:
        ...
        return not self.is_in_grim_forest_mode()
