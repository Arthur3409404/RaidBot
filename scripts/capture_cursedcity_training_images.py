...
from raid_bot.modes.cursedcity_tools import RSL_Bot_CursedCity, _spiral_direction_for_step
from raid_bot.utils import map_tools, window_tools


DEFAULT_OUTPUT_ROOT = Path("data") / "output" / "debug" / "cursedcity_zoomed_out"
...
def _cursed_city_mask(pov_np: np.ndarray, target_rgb: np.ndarray) -> np.ndarray:
    base = target_rgb.astype(np.int16)
    diff = pov_np.astype(np.int16) - base
    brighter_tones_mask = (diff[:, :, 0] >= 0) & (diff[:, :, 1] >= 0) & (diff[:, :, 2] >= 0)
    return np.where(brighter_tones_mask, 255, 0).astype(np.uint8)
...
def _serpentine_movements(repeats: int = 3, horizontal_steps: int = 4) -> list[str]:
    movements: list[str] = []
...
def _save_capture(
    *,
    ...
    mask = _cursed_city_mask(pov_np, bot.target_bgr_as_rgb)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Capture non-overwriting Cursed City detector training images after zooming out. "
    ...
    parser.add_argument("--output-root", default=str(DEFAULT_OUTPUT_ROOT), help="Output collection root.")
    ...
    parser.add_argument(
        ...
        default="none",
        help="Optionally switch Cursed City difficulty before zoom/capture.",
    )
    ...
    parser.add_argument("--reset-left-steps", type=int, default=8, help="Left moves used for bottom-left reset.")
    parser.add_argument("--reset-down-steps", type=int, default=8, help="Down moves used for bottom-left reset.")
    parser.add_argument(
        "--movement",
    ...
    parser.add_argument("--serpentine-repeats", type=int, default=3, help="Number of serpentine row pairs.")
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    bot = RSL_Bot_CursedCity(reader=None, window=window, setup={"initial_candidate_zoom_out_steps": args.zoom_steps})
    relative_area = bot.search_areas["pov"]

    print(f"Output: {session_dir}")
    print(f"Window: left={window.left} top={window.top} width={window.width} height={window.height}")
    if args.difficulty != "none":
        print(f"Switching difficulty to {args.difficulty}.")
        _switch_difficulty(
            bot,
            args.difficulty,
    ...
    print(f"Zooming out {args.zoom_steps} step(s).")
    ...
    for index in range(total_captures):
        ...
        label = f"cursedcity_zoomed_out_{index + 1:03d}"
        ...
        print(f"[{index + 1}/{total_captures}] {run_dir}")
    ...
    print(f"Done. Manifest: {session_dir / 'manifest.json'}")
    return 0
