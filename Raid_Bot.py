...
def _resolve_run_bot_script():
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), RUN_BOT_SCRIPT_NAME)


def _spawn_run_bot_process(*extra_args: str, env: dict | None = None):
    command = [sys.executable, _resolve_run_bot_script(), *extra_args]
    return subprocess.Popen(command, cwd=os.path.dirname(_resolve_run_bot_script()), env=env)
...
def run_restart_helper(parent_pid: int) -> int:
    ...
    subprocess.Popen(
        _build_raid_launch_command(),
        cwd=os.path.dirname(_resolve_run_bot_script()),
    )

    if not _wait_for_raid_window(timeout_seconds=180.0):
        raise TimeoutError("Raid window did not appear within 180 seconds during restart.")
...
import data.lib.handlers.raid_calendar_handler as raid_calendar_handler
from data.lib.core import BotCommandRouter, runtime_discord, runtime_launch, runtime_reporting
...
class RSL_Bot_Mainframe:
    ...
    def __init__(self, title_substring="Raid: Shadow Legends"):
        ...
        self.daily_log_path = file_tools.get_daily_log_path(datetime.now(BERLIN_TZ))
        ...
        self._append_daily_log_lines(
            [
                "",
                f"[{datetime.now(BERLIN_TZ).strftime('%Y-%m-%d %H:%M:%S')}] Mainframe started",
    ...
    def restart_raid_process(self):
        ...
        try:
            subprocess.Popen(self.raid_path)
        except FileNotFoundError as exc:
        ...
        while not gw.getWindowsWithTitle(RAID_WINDOW_TITLE):
            if time.time() - wait_start > 180:
                raise TimeoutError("Raid window did not appear within 180 seconds after restart.")
            time.sleep(2)
        time.sleep(60)
...
if __name__ == "__main__":
    ...
    # run_classic_arena = False
    # run_tagteam_arena = False
    ...
    #
    # # DEBUG: Record relative squares for search_areas (disabled by default)
    num_squares = 0
    dbg_windows = gw.getWindowsWithTitle(RAID_WINDOW_TITLE)
    if dbg_windows:
        ...
        for i in range(num_squares):
            input(f"[{i + 1}/{num_squares}] Move mouse to TOP-LEFT, then Enter...")
            x1, y1 = pyautogui.position()
            input(f"[{i + 1}/{num_squares}] Move mouse to BOTTOM-RIGHT, then Enter...")
        ...
        print(f"Window not found: {RAID_WINDOW_TITLE}")

    # DEBUG: Optional Plarium window catcher + relative-square recorder.
    # Set to True when you want to calibrate/search coordinates against Plarium Play.
    debug_plarium_window_capture = False
    if debug_plarium_window_capture:
        if not gw.getWindowsWithTitle(PLARIUM_WINDOW_TITLE):
            plarium_path, checked_paths = _resolve_plarium_play_executable()
            if plarium_path:
                subprocess.Popen([plarium_path])
            else:
                checked = ", ".join(checked_paths) if checked_paths else "<none>"
                print(
                    "Could not locate PlariumPlay.exe for debug launch. "
                    f"Checked: {checked}"
                )

        while not gw.getWindowsWithTitle(PLARIUM_WINDOW_TITLE):
            time.sleep(2)

        plarium_windows = gw.getWindowsWithTitle(PLARIUM_WINDOW_TITLE)
        plarium_win = plarium_windows[0]
        plarium_win.moveTo(10, 10)
        time.sleep(2)

        num_squares_plarium = 8
        dbg_windows = gw.getWindowsWithTitle(PLARIUM_WINDOW_TITLE)
        if dbg_windows:
            dbg_win = dbg_windows[0]
            dbg_left, dbg_top, dbg_width, dbg_height = (
                dbg_win.left, dbg_win.top, dbg_win.width, dbg_win.height
            ...
            dbg_coords = (dbg_left, dbg_top, dbg_width, dbg_height)
            print(f"Plarium window.left/top/width/height = {dbg_coords}")
            for i in range(num_squares_plarium):
                input(f"[Plarium {i + 1}/{num_squares_plarium}] Move mouse to TOP-LEFT, then Enter...")
                x1, y1 = pyautogui.position()
                input(f"[Plarium {i + 1}/{num_squares_plarium}] Move mouse to BOTTOM-RIGHT, then Enter...")
                x2, y2 = pyautogui.position()
                left, right = sorted((x1, x2))
                top, bottom = sorted((y1, y2))
                rel = [
                ...
                print(f"plarium_square_{i + 1}: {rel}")
        else:
            print(f"Window not found: {PLARIUM_WINDOW_TITLE}")
